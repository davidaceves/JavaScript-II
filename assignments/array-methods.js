...
// I used .map() below by accident.

// fullName = runners.map((full_name) => {
...
allCaps = runners.map((fName_caps) => {
    return `${fName_caps.first_name.toUpperCase()}`;
});
...
largeShirts = runners.filter(runner_shirt => runner_shirt.shirt_size === 'L');
...
ticketPriceTotal = runners.reduce((total, runner_donation) => {
    return total + runner_donation.donation;    
}, 0);
...
// Problem 1
// Find people who donated more than $100 and print thier full name plus donation amount.

let bigSpenders = [];

const richRunner = runners.filter(largeDonation => largeDonation.donation >= 100);

richRunner.forEach((name) => {
    bigSpenders.push(`${name.first_name} ${name.last_name} $${name.donation}`);
})


console.log(JSON.stringify(bigSpenders));

// Problem 2
// Go through each donation and double, to simulate a large company matching the runners donations. Although it would be simpler to just double the total value. 

let matchDonation = [];

matchDonation = runners.map((doner_match) => {
    return `Your new matched donation is: ${doner_match.donation * 2}`;
});

console.log(matchDonation);

// Problem 3
// Function that allows you to search for runners by the first letter of their last name, it then returns the last name followed by the first name.

function filterLastNames (letter) {
    let filterByLetter = [];
    const filteredNames = [];
    
    filterByLetter = runners.filter(lastName => lastName.last_name.charAt(0) === letter);

    filterByLetter.forEach((name) => {
        filteredNames.push(`${name.last_name}, ${name.first_name} `);
    })
